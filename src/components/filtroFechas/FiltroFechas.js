...
const FiltroFechas = () => {
  ...
  }, [inf.startDate, inf.endDate, tdFechas, todasLasCabañas]);
...
const FiltroFechas = () => {
  ...
  const fechaDesde = inf.startDate ? DateTime.fromISO(inf.startDate) : null;
  const fechaHasta = inf.endDate ? DateTime.fromISO(inf.endDate) : null;

  const tdFechas = useMemo(() => {
    const result = [];
    if (fechaDesde && fechaHasta) {
      const daysDiff = fechaHasta.diff(fechaDesde, 'days').toObject().days + 1;
      ...
      for (let i = 0; i < daysDiff; i++) {
        const date = fechaDesde.plus({ days: i });
        result.push(date.toJSDate().toDateString());
      }
    }
    return result;
  }, [fechaDesde, fechaHasta]);

  const todasLasCabañas = inf.data;
          ...
          <>
            {cabañasDisponibles.length > 0 ? (
              <>
                ...
                {cabañasDisponibles.map((cabaña) => (
                  <div key={cabaña.id} className='cabaña-item'>
                    <Link to={`/cabaña/${cabaña.id}`}>{cabaña.name}</Link>
                  </div>
                ))}
              </>
            ) : (
              <h4>No hay cabañas disponibles para las fechas seleccionadas.</h4>
            )}
          </>
        )}
      </div>
...
const FiltroFechas = () => {
  ...
  const [cabañasDisponibles, setCabañasDisponibles] = useState([]);
  const [tdFechas, setTdFechas] = useState([]);
  ...
  if (fechaDesde && fechaHasta) {
    ...
    for (let i = 0; i < daysDiff; i++) {
      const date = fechaDesde.plus({ days: i });
      //tdFechas.push(date.toJSDate().toDateString()); 
      setTdFechas(date.toJSDate().toDateString())
    }
  }
